...
public class OverrideComissionsAffiliateTest extends WebDriverUtility {

	String successPage = new String("Login to PayKickstart");
	User affiliate = new User("testPay@mailinator.com", "Test123");
	User lenjaVendor = new User("lenjatest@gmail.com", "Test123");
	...
	public void verifyCreateAffiliateLinkTest(User user) throws MalformedURLException, InterruptedException {
		...
		startPage.openAffiliatesPage();
		affPage.findAffiliate();
		affPage.editAffiliate();
		affPage.getToComissSettings(sameCampaign);
		affPage.overrideSettings();
		affPage.getToComissSettings(sameCampaign);
		affPage.checkCommissSaved();
		driver.quit();
	}
